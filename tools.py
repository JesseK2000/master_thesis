...
def loss_mse_oh(pred_conf, pred_cls, pred_txtytwth, label, cls_embeddings):
    ...
    cls_loss_function = nn.MSELoss(reduction='none')
    ...
    gt_embeddings = cls_embeddings[gt_cls.long()]
    
    # cls_loss_function(pred_cls, gt_embeddings) has shape (32,400,20),
    # gt_obj.unsqueeze(-1) has shape (32,400,1)

    cls_loss = torch.mean(torch.sum(cls_loss_function(pred_cls, gt_embeddings) * gt_obj.unsqueeze(-1), axis = (1,2)))
...
def loss_mse_eud(pred_conf, pred_cls, pred_txtytwth, label, cls_embeddings):
...
def loss_mse_hyp(pred_conf, pred_cls, pred_txtytwth, label, cls_embeddings):
...
def loss(pred_conf, pred_cls, pred_txtytwth, label):
    ...
    while torch.isinf(cls_loss) or torch.isnan(cls_loss):
        cls_loss = torch.mean(torch.sum(cls_loss_function(pred_cls, gt_cls) * gt_obj, 1))
